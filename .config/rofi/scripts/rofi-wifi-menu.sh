...
# Configuration
THEME_FILE="$HOME/.config/rofi/wifi.rasi"
LOCK_SYMBOL="🔒"
UNLOCK_SYMBOL="🔓"
REFRESH_SYMBOL="🔄"
SETTINGS_SYMBOL="⚙️"
...
connect_to_network() {
    ...
    local bssid=$(echo "$selection" | grep -o -E "([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}")
    ...
    local security=$(nmcli -f SSID,SECURITY dev wifi list | grep "$ssid" | awk '{print $2}')
